...
namespace testing{
    ...
    void avp64_testing_receiver::notify_breakpoint_hit(const vcml::debugging::breakpoint& bp){
        ...
        if(m_run_until_breakpoint && bp.address() != m_run_until_breakpoint_addr){
            log_info_message("Ignoring breakpoint, because in run until mode.");
            return;
        }
            ...
            *addr = sym_ptr->virt_addr()+1;
            ...
            if(set_breakpoint){
                if(!static_cast<vcml::debugging::target*>(target_core)->insert_breakpoint(*addr, this)){
                    log_error_message("Failed to insert the breakpoint at symbol %s (address %d)!", name.c_str(), *addr);
                }
            }
            return true;
        }
    ...
    status avp64_testing_receiver::handle_do_run(std::string &start_breakpoint, std::string &end_breakpoint, uint64_t mmio_address, size_t mmio_length, char* mmio_value, string &register_name){   
        ...
        if(start_breakpoint != ""){
            ...
            bool jump_success = target_core->jump_to(m_run_until_breakpoint_addr-1);
